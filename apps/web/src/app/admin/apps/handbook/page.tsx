...
export default function AdminHandbookPage() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const cfg = await apiFetch<AppConfig>('/admin/apps/handbook');
        setApp(cfg);
      } catch {
        setApp(null);
      }
  ...
  async function save(patch: Partial<AppConfig>) {
    ...
    setSaving(true);
    try {
      const updated = await apiFetch<AppConfig>('/admin/apps/handbook', {
        method: 'PATCH',
  ...
  return (
    <main style={{ padding: 32, maxWidth: 720 }}>
      <h1 style={{ marginTop: 0 }}>应用 · 手册</h1>
      <p style={{ color: 'var(--muted)' }}>
        个人工作流程笔记：分类树、Markdown 正文与库内搜索。上架后成员可在工作台使用。
      </p>
      ...
      <p style={{ marginTop: 24 }}>
        <a href="/workbench/apps/handbook" style={{ color: 'var(--accent)', fontWeight: 600 }}>
          打开手册 →
        </a>
      </p>
    </main>
