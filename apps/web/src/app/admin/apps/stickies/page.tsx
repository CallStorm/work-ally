...
export default function AdminStickiesPage() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const cfg = await apiFetch<AppConfig>('/admin/apps/stickies');
        setApp(cfg);
      } catch {
        setApp(null);
      }
  ...
  async function save(patch: Partial<AppConfig>) {
    ...
    setSaving(true);
    try {
      const updated = await apiFetch<AppConfig>('/admin/apps/stickies', {
        method: 'PATCH',
  ...
  return (
    <main style={{ padding: 32, maxWidth: 720 }}>
      <h1 style={{ marginTop: 0 }}>应用 · 闪签（日历任务）</h1>
      <p style={{ color: 'var(--muted)' }}>
        以日 / 周 / 月日历为中心的个人任务：优先级、提醒、拖拽改期。上架后成员可在工作台使用。
      </p>
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: 13, color: 'var(--muted)' }}>
          本应用不再使用默认模型与 AI 动作；相关字段仍保留在 API，管理端已隐藏。
        </p>
      ...
      <p style={{ marginTop: 24 }}>
        <a href="/workbench/apps/stickies" style={{ color: 'var(--accent)', fontWeight: 600 }}>
          打开日历任务 →
        </a>
      </p>
    </main>
